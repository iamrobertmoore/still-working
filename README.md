...
Come back during judging: the [daily page](https://iamrobertmoore.github.io/still-working/) keeps updating from scheduled runs and shows the supplier-check time alongside the agent-review status.
...
<!-- measurement:start -->
**170 days. 25 supplier-change records across 24 calendar days. 77 individual changes. 11 potentially breaking changes.**
...
Two interruptions in 5 months of replay, from 77 supplier changes. That is **2.6%**. **That ratio is the product.** [Method and scope](#evidence-and-scope).

_Window 2026-04-07 to 2026-09-24. Regenerated by the scheduled job on every run, never typed by hand. Re-run it with `python tools/impact.py --measure`._
<!-- measurement:end -->
...
**Collect what changed.** A credential-free daily job compares the contracts published by Stripe, Square, Xero and ShipEngine. It recognises structural changes that can break an existing caller.
...
**Keep control of delivery.** Strands `Transform` stamps known factual fields; `Deny` blocks ineligible delivery attempts. Local `Confirm` supports human approval. The deployed path holds uncertain cases before building a model, then accepts a case-specific review from its authenticated caller. A separate cloud job persists successful notes and their ledger. Collection publishes first and queues relevant changes, so an AWS failure leaves them available for retry. The page reports supplier freshness and agent-review status separately.
...
|---|---|---|---|
| how customers pay me | Stripe | 612 | `2026-09-30.endive` |
| my till and my bookings | Square | 332 | `2.0` |
...
The fresh demo matches the historical Square stock change, invokes AWS to render a new note, then carries the returned ledger into a second invocation. The repeat must produce no model run and no second note. It uses an isolated ledger and leaves the published replay untouched. [Recording instructions](docs/LIVE-DEMO.md).

To regenerate the full historical replay instead:
