...
def wrong_dates_in(text: str, allowed_iso: str | None) -> set[tuple[int, int]]:
    ...
    try:
        ...
        return set()
    wrong = dates_mentioned(text) - {(m, d)}
    # Month and day alone cannot catch the model changing the year.
    for iso, *_ in _DATE_IN_TEXT.findall(text or ""):
        if iso and iso != allowed_iso:
            _, written_month, written_day = iso.split("-")
            wrong.add((int(written_month), int(written_day)))
    return wrong
