...
from tools.render_page import CSS,mark,human_date,masthead
...
def render(case, mode, proof=None):
    ...
    explanation=('An uncertain match pauses the agent. Confirm it, dismiss it, or leave the question open. Explore the recorded outcomes.' if example else 'A detected supplier change is waiting for someone who knows the integration. Record the check, download your decision, and give it to the project operator to apply.')
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Review a match | Still Working</title><link rel="icon" href="../brand/mark.svg"><style>{CSS}\n{REVIEW_CSS}</style></head><body><div class="wrap">{masthead("../")}<nav class="review-nav"><a href="../">Daily check-in</a><a href="../replay/">Recorded warnings</a><a href="live.html">Pending queue</a></nav>
    ...
    <footer class="review-footer"><p><a href="{'proof.json' if example else 'pending.json'}">Inspect the source record</a><a href="https://github.com/iamrobertmoore/still-working/blob/main/docs/REVIEW.md">How the review reaches the agent</a></p><details class="demo-method"><summary>About {'this example' if example else 'the review process'} <span aria-hidden="true">+</span></summary><div><p>{'This uses a historical Square change with deliberately lowered mapping confidence and simulated reviewer choices. Each branch is a saved AWS response. Maya’s shop and its business consequences are illustrative. Example files cannot be imported into the live queue.' if example else 'A repository operator imports the downloaded file; the daily caller then applies the decision. Nothing is submitted from this page. Reviewer names are supplied, not identity-verified.'}</p><p>Each review covers the exact date, supplier and assessed routine. Changing the case invalidates the decision.</p></div></details></footer></div><script id="review-data" type="application/json">{encoded}</script><script src="review.js"></script></body></html>'''
...
def main():
    ...
    (out/'live.html').write_text(f'<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Pending reviews | Still Working</title><style>{CSS}{REVIEW_CSS}</style></head><body><div class="wrap">{masthead("../")}<main class="review-head"><p class="eyebrow">Published review queue</p><h1>Waiting for a person.</h1><div class="case-queue">{body}</div></main></div></body></html>')
