...
def page(data, selected):
    ...
    if note:
        ...
        body = '<div class="empty-note"><h2>' + ('You have already been told.' if repeat else 'Still working.') + '</h2><p>' + ('The payroll routine was flagged five days earlier. The delivery ledger holds this repeat before a model is built. One useful warning is already on the list.' if repeat else 'These published changes do not match a potentially breaking change to a routine in the shop profile. No note. No model invocation.') + '</p></div>'
    ...
    </main><footer class="replay-footer"><p><strong>{m['changes']} supplier changes. {data['delivered_notes']} notes. That ratio is the product.</strong></p><p><a href="../review/">Try a review decision ↗</a> · <a href="recordings.json">Download the evidence</a> · <a href="https://github.com/iamrobertmoore/still-working/blob/main/tools/replay.py">Reproduce it</a></p>
    <details class="demo-method"><summary>About this replay <span aria-hidden="true">+</span></summary><div><p>Captured {esc(data['captured_at'][:10])} from AgentCore: {len(data['cases'])} supplier-day records across {len({c['record']['date'] for c in data['cases']})} dates, spanning {m['span']} days. These are saved responses. Maya’s shop and its dependencies are illustrative; supplier publications are real. The profile was informed by this history.</p><p>The ratio measures notification volume, not measured accuracy or customer savings. Relative dates refer to each historical event. Original model wording is preserved. A held repeat is not proof the issue is resolved. One agent run may include several model requests; delivery means a rendered note, not an email to a customer.</p><a href="https://github.com/iamrobertmoore/still-working#evidence-and-scope">Full evidence and scope ↗</a></div></details></footer></div></body></html>'''
