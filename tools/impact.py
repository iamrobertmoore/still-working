...
def figures(business: dict | None = None) -> dict:
    ...
    return {
        ...
        "revisions": len(rows), "changed_days": len(set(dates)), "changes": total,
...
def readme_block(business: dict | None = None) -> str:
    ...
    lines = [
        README_START,
        f"**{f['span']} days. {f['revisions']} supplier-change records across {f['changed_days']} calendar days. "
        f"{f['changes']} individual changes. {f['breaking']} potentially breaking changes.**",
        "",
        f"{f['touched']} supplier-change records touched calls in Maya's illustrative profile.",
        "",
        f"**{f['reaching']} records contained a potentially breaking change to a mapped routine. "
        f"The replay would interrupt her {_words(f['interruptions'])}.**",
        "",
    ...
    lines += [
        ...
        f"months of replay, from {f['changes']} supplier changes. That is **{f['ratio']:.1f}%**. "
        "That ratio is the product. It measures notification volume for this profile, "
        "not accuracy, prevented outages or money saved for a real customer.",
        "",
